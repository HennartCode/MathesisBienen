...
# fonts
pause_text = pygame.font.SysFont("Comic Sans", 32).render(
...
for _ in range(config.HIVES):
    add_sprite(
        hive.Hive(uniform(10, config.WIDTH - 10), uniform(10, config.HEIGHT - 10)),
        groups.hives,
    )
...
def main():
    ...
    while running:
        ...
        if state == config.MAIN:
            ...
            for bee in groups.bees:
                bee.draw(screen)
            groups.flowers.draw(screen)
            # bees.update(flowers,bees)
            for f in groups.flowers:
                groups.bees.update(f, groups.bees)
                # bees.update(prop_flower_list[1],bees)
            groups.flowers.update(screen)
            groups.hives.draw(screen)
